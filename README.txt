Attach LMStudioClient.cs to a GameObject in a Unity scene. Fill out the URL and Model fields. These, along with the System Prompt and User Prompt, are adjustable in the inspector.

On Start(), the script will immediately attempt to make a request to LM Studio unless you disable this behavior. Check the Unity console for successful output.

Ensure the LM Studio server is running first. On Mac, go to the "Local Server" tab in LM Studio, and look for the green "Start Server" button (assuming LM Studio Version 0.2.24 (0.2.24)). You can also find the URL/endpoint in this tab.
